...
class NLVO
{
    ...
    // Global variables
    const float control_limit_x = 5.0f; // |u_x| <= control_limit_x
    const float control_limit_y = 5.0f; // |u_y| <= control_limit_y
    ...
    twist_msg selectBestVelocity(const pose_msg &ego_pose, const twist_msg &ego_vel, const std::vector<Obstacle>& obstacles_, const point_msg &goal_point, float r_total);
